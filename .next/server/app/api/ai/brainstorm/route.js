...
Keep responses concise. Always end with a question.`},h=d[i]||d.show_skill,m=u.map(e=>({role:e.role,content:e.content})),f=new AbortController,g=setTimeout(()=>f.abort(),25e3);try{s=await t.messages.create({model:"claude-sonnet-4-20250514",max_tokens:300,system:h,messages:m},{signal:f.signal})}catch(e){if("AbortError"===e.name)return a.NextResponse.json({message:"Still thinking... The AI is taking longer than usual. Please try again.",sessionType:i});return console.error("[BRAINSTORM] Anthropic error:",e.message||"Unknown"),(0,l.Vd)("AI service temporarily unavailable")}finally{clearTimeout(g)}let w="text"===s.content[0].type?s.content[0].text:"";return a.NextResponse.json({message:w,sessionType:i})}catch(e){return console.error("[BRAINSTORM] Error:",e instanceof Error?e.message:"Unknown"),(0,l.Vd)("Failed to process brainstorm request")}}let d=new n.AppRouteRouteModule({definition:{kind:o.x.APP_ROUTE,page:"/api/ai/brainstorm/route",pathname:"/api/ai/brainstorm",filename:"route",bundlePath:"app/api/ai/brainstorm/route"},resolvedPagePath:"/workspace/leadership-platform/src/app/api/ai/brainstorm/route.ts",nextConfigOutput:"",userland:s}),{requestAsyncStorage:h,staticGenerationAsyncStorage:m,serverHooks:f}=d,g="/api/ai/brainstorm/route";function w(){return(0,i.patchFetch)({serverHooks:f,staticGenerationAsyncStorage:m})}},4596:(e,t,r)=>{r.d(t,{Cp:()=>b,Dn:()=>w,Gu:()=>u,M2:()=>m,Py:()=>p,Vd:()=>h,bQ:()=>i,fp:()=>g,h3:()=>a,ks:()=>d,m:()=>c,ni:()=>y,oN:()=>l,pG:()=>k,tm:()=>x,uw:()=>o,vJ:()=>v});var s=r(8635),n=r(2380);function o(e){let t="https://apakkhzuydsfzvypewwa.supabase.co",r="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImFwYWtraHp1eWRzZnp2eXBld3dhIiwicm9sZSI6ImFub24iLCJpYXQiOjE3Njg1MDEyODQsImV4cCI6MjA4NDA3NzI4NH0.c3hiZOEKMIXMP7aB0SVgrbQH58nwxseycVmeH23KsNQ";if(!t||!r)throw Error("Missing Supabase configuration");let s=e.headers.get("Authorization");return(0,n.eI)(t,r,{global:{headers:s?{Authorization:s}:{}}})}let i=function(){let e="https://apakkhzuydsfzvypewwa.supabase.co",t=process.env.SUPABASE_SERVICE_ROLE_KEY;if(!e||!t)throw Error("Missing Supabase service configuration");return(0,n.eI)(e,t)};async function a(e){try{let t=o(e),{data:{user:r},error:s}=await t.auth.getUser();if(s||!r)return{authenticated:!1,error:"Invalid or expired session"};let{data:n,error:i}=await t.from("users").select("id, role, class_id").eq("id",r.id).single();if(i||!n)return{authenticated:!1,error:"User profile not found"};return{authenticated:!0,userId:r.id,role:n.role,classId:n.class_id}}catch(e){return console.error("[AUTH] Validation error:",e instanceof Error?e.message:"Unknown"),{authenticated:!1,error:"Authentication failed"}}}async function u(e){let t=await a(e);return t.authenticated&&"student"!==t.role?{authenticated:!1,error:"Student access required"}:t}async function l(e,t){let r=await a(e);if(!r.authenticated)return r;if("teacher"===r.role){let s=o(e),{data:n}=await s.from("classes").select("id").eq("id",t).eq("teacher_id",r.userId).single();if(!n)return{authenticated:!1,error:"Not authorized for this class"}}else if(r.classId!==t)return{authenticated:!1,error:"Not authorized for this class"};return r}function c(e="Unauthorized"){return s.NextResponse.json({error:e},{status:401})}function p(e="Forbidden"){return s.NextResponse.json({error:e},{status:403})}function d(e){return s.NextResponse.json({error:e},{status:400})}function h(e="Internal server error"){return s.NextResponse.json({error:e},{status:500})}function m(e){return s.NextResponse.json({error:e},{status:503})}let f=new Map,g={ai:{windowMs:6e4,maxRequests:10},standard:{windowMs:6e4,maxRequests:60},write:{windowMs:6e4,maxRequests:30}};function w(e,t=g.standard){let r=Date.now();f.size>1e4&&f.forEach((e,t)=>{e.resetAt<r&&f.delete(t)});let s=f.get(e);return!s||s.resetAt<r?(f.set(e,{count:1,resetAt:r+t.windowMs}),{allowed:!0,remaining:t.maxRequests-1,resetIn:t.windowMs}):s.count>=t.maxRequests?{allowed:!1,remaining:0,resetIn:s.resetAt-r}:(s.count++,{allowed:!0,remaining:t.maxRequests-s.count,resetIn:s.resetAt-r})}function x(e){return s.NextResponse.json({error:"Too many requests. Please slow down."},{status:429,headers:{"Retry-After":String(Math.ceil(e/1e3)),"X-RateLimit-Reset":String(Math.ceil(e/1e3))}})}function y(e){return"string"==typeof e&&/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(e)}function b(e,t=5e3){if("string"!=typeof e)return!1;let r=e.trim();return r.length>0&&r.length<=t}function k(e){return"string"==typeof e&&["do_now","scenario","challenge","exit_ticket"].includes(e)}function v(e){return e.replace(/<[^>]*>/g,"").replace(/[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]/g,"").trim()}}};var t=require("../../../../webpack-runtime.js");t.C(e);var r=e=>t(t.s=e),s=t.X(0,[469,16,654],()=>r(596));module.exports=s})();